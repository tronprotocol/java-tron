...
public class StateArchiveHotProcessRecoveryTest {
  ...
  public void jvmHaltAfterHotPrepareBeforeCommonWalTruncatesOrphanOnRestart()
      throws Exception {
    Path root = temporaryFolder.newFolder("hot-process-recovery").toPath();
    ...
    try (StateArchiveHotStore store = open(root.resolve("hot"))) {
    ...
    try (StateArchiveHotStore reopened = open(root.resolve("hot"))) {
  ...
  public static void main(String[] args) throws Exception {
    if (args.length != 2 || !"halt-after-prepare".equals(args[0])) {
  ...
  private static StateArchiveHotStore open(Path root) throws Exception {
    return StateArchiveHotStore.openOrCreate(root, FORMAT, Engine.LEVELDB, 0, BASE_HASH,
        3, 10, 1024 * 1024);
  }

  private static BlockReverseDiff diff(long block, int parent) {
    return new BlockReverseDiff(meta(block, parent), Collections.singletonList(
        new DbGroup("code", Collections.singletonList(
            new Entry(new byte[]{1}, OldValue.absent())))), hash(60 + (int) block));
  }
