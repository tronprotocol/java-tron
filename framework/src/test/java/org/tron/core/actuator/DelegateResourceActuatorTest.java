...
public class DelegateResourceActuatorTest {
  ...
  private Any getDelegateContractForBandwidth(
      String ownerAddress, String receiveAddress, long unfreezeBalance) {
    return Any.pack(
  ...
  private Any getDelegateContractForCpu(
      String ownerAddress, String receiveAddress, long unfreezeBalance) {
    return Any.pack(
        DelegateResourceContract.newBuilder()
            .setOwnerAddress(ByteString.copyFrom(ByteArray.fromHexString(ownerAddress)))
  ...
  private Any getDelegateContractForTronPower(
      String ownerAddress, String receiveAddress, long unfreezeBalance) {
    return Any.pack(
        DelegateResourceContract.newBuilder()
            .setOwnerAddress(ByteString.copyFrom(ByteArray.fromHexString(ownerAddress)))
  ...
  public void testDelegateResourceWithNoFreeze() {
    ...
    try {
      ...
      Assert.assertEquals("delegateBalance must be less than accountFreezeBandwidthBalance",
          e.getMessage());
    ...
    actuator.setChainBaseManager(dbManager.getChainBaseManager()).setAny(
        getDelegateContractForCpu(OWNER_ADDRESS, RECEIVER_ADDRESS, 1_000_000_000L));
    try {
      actuator.validate();
      actuator.execute(ret);
    } catch (ContractValidateException e) {
      Assert.assertEquals("delegateBalance must be less than accountFreezeEnergyBalance",
          e.getMessage());
    } catch (ContractExeException e) {
  ...
  public void testDelegateResourceForBandwidth() {
    ...
    try {
      ...
      byte[] key = DelegatedResourceAccountIndexCapsule.createDbKeyV2(owner);
  ...
  public void testDelegateResourceForCpu() {
    ...
    DelegateResourceActuator actuator = new DelegateResourceActuator();
    actuator.setChainBaseManager(dbManager.getChainBaseManager()).setAny(
        getDelegateContractForCpu(OWNER_ADDRESS, RECEIVER_ADDRESS, delegateBalance));
    ...
    try {
      ...
      Assert.assertEquals(0L, ownerCapsule.getDelegatedFrozenBalanceForBandwidth());
