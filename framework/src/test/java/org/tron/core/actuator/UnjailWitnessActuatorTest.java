...
@Slf4j
public class UnjailWitnessActuatorTest {

  private static final String dbPath = "output_UnjailWitness_test";
  ...
  private Any getContract(String address, String srAddress) {
    return Any.pack(
            UnjailWitnessContract.newBuilder()
                    .setOwnerAddress(ByteString.copyFrom(ByteArray.fromHexString(address)))
                    .setWitnessAddress(ByteString.copyFrom(ByteArray.fromHexString(srAddress)))
                    .build());
  }

  private Any getContract(String address, ByteString srAddress) {
    return Any.pack(
            UnjailWitnessContract.newBuilder()
                    .setOwnerAddress(ByteString.copyFrom(ByteArray.fromHexString(address)))
                    .setWitnessAddress(srAddress)
                    .build());
  }
   ...
   */
  @Test
  public void rightUnjailWitness() {
    WitnessCapsule witness =
            dbManager.getWitnessStore().get(ByteArray.fromHexString(OWNER_ADDRESS));
    witness.setJailedHeight(100);
    dbManager.getWitnessStore().put(ByteArray.fromHexString(OWNER_ADDRESS), witness);
    UnjailWitnessActuator actuator = new UnjailWitnessActuator();
    ...
    try {
      ...
      Assert.assertEquals(witnessCapsule.getJailedHeight(), 0);
  ...
  public void InvalidAddress() {
    UnjailWitnessActuator actuator = new UnjailWitnessActuator();
    actuator.setChainBaseManager(dbManager.getChainBaseManager())
            .setAny(getContract(OWNER_ADDRESS_INVALID, OWNER_ADDRESS));
    TransactionResultCapsule ret = new TransactionResultCapsule();
    try {
      actuator.validate();
      actuator.execute(ret);
   ...
   */
  @Test
  public void InvalidJailedHeightTest() {
    WitnessCapsule witness =
            dbManager.getWitnessStore().get(ByteArray.fromHexString(OWNER_ADDRESS));
    witness.setJailedHeight(0);
    dbManager.getWitnessStore().put(ByteArray.fromHexString(OWNER_ADDRESS), witness);
    TransactionResultCapsule ret = new TransactionResultCapsule();
    //Url cannot empty
    try {
      ...
      actuator.validate();
      actuator.execute(ret);
      fail("Witness already unjailed");
    } catch (ContractValidateException e) {
      Assert.assertTrue(e instanceof ContractValidateException);
      Assert.assertEquals("Witness already unjailed", e.getMessage());
    } catch (ContractExeException e) {
      Assert.assertFalse(e instanceof ContractExeException);
    }
  }
  ...
  public void notExistWitness() {
    UnjailWitnessActuator actuator = new UnjailWitnessActuator();
    actuator.setChainBaseManager(dbManager.getChainBaseManager())
            .setAny(getContract(OWNER_ADDRESS, OWNER_ADDRESS_NOT_WITNESS));
    TransactionResultCapsule ret = new TransactionResultCapsule();
    try {
      actuator.validate();
      actuator.execute(ret);
  ...
  public void notExistAccount() {
    UnjailWitnessActuator actuator = new UnjailWitnessActuator();
    actuator.setChainBaseManager(dbManager.getChainBaseManager())
            .setAny(getContract(OWNER_ADDRESS_NOTEXIST, OWNER_ADDRESS));
    TransactionResultCapsule ret = new TransactionResultCapsule();
    try {
      actuator.validate();
      actuator.execute(ret);
  ...
  public void commonErrorCheck() {
    ...
    actuatorTest.setInvalidContractTypeMsg("contract type error",
            "contract type error, expected type [UnjailWitnessContract], real type[");
    actuatorTest.invalidContractType();

    actuatorTest.setContract(getContract(OWNER_ADDRESS, OWNER_ADDRESS));
