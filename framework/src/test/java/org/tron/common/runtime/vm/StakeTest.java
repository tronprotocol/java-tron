...
contract TestStake{
  ...
  function UnStake() public returns (bool result){
    ...
    public void testStake() throws Exception{
        ...
        Assert.assertEquals(contract.getFrozenCount(), 1);
        Assert.assertEquals(contract.getFrozenBalance(), 10000000);
//        Assert.assertTrue(contract.getFrozenList().get(0).getExpireTime() > System.currentTimeMillis() - 600000);
        Assert.assertEquals(contract.getVotesList().get(0).getVoteCount(), 5);

        // vote not exist witness
        hexInput = AbiUtil.parseMethod(methodByAddr, Arrays.asList("27k66nycZATHzBasFT9782nTsYWqVtxdtAc", 20000000));
        result = TvmTestUtils
                .triggerContractAndReturnTvmTestResult(Hex.decode(OWNER_ADDRESS),
                        factoryAddress, Hex.decode(hexInput), 0, feeLimit, manager, null);
        Assert.assertNull(result.getRuntime().getRuntimeError());
        ...
        // param error
        hexInput = AbiUtil.parseMethod(methodByAddr, Arrays.asList("27k66nycZATHzBasFT9782nTsYWqVtxdtAc", 2000000000));
        result = TvmTestUtils
                .triggerContractAndReturnTvmTestResult(Hex.decode(OWNER_ADDRESS),
                        factoryAddress, Hex.decode(hexInput), 0, feeLimit, manager, null);
        Assert.assertNull(result.getRuntime().getRuntimeError());
