...
public class FreezeV2Test {
  ...
  private static final String FREEZE_V2_CODE =
      "6080604052610e85806100136000396000f3fe60806040526004361061010d5760003560e01c80635897454711"
  ...
  private TVMTestResult unDelegateResource(
    ...
    if (oldReceiver != null) {
      ...
      if (res == 0) {
        ...
        Assert.assertEquals(
            ChainBaseManager.getInstance().getHeadSlot(), newReceiver.getLastConsumeTime(BANDWIDTH));
      } else {
        Assert.assertEquals(
            oldReceiver.getEnergyUsage() + transferUsage, newReceiver.getEnergyUsage());
        Assert.assertEquals(
            ChainBaseManager.getInstance().getHeadSlot(), newReceiver.getLastConsumeTime(ENERGY));
      }
    } else {
