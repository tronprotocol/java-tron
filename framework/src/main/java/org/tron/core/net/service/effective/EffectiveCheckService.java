...
public class EffectiveCheckService {
  ...
  private final String name = "effective-thread";
  private ScheduledExecutorService executor = ExecutorServiceManager
  ...
  public void close() {
    if (executor != null) {
      try {
        logger.info("EffectiveCheck shutdown...");
        ExecutorServiceManager.shutdownAndAwaitTermination(executor, name);
        logger.info("EffectiveCheck shutdown complete");
      } catch (Exception e) {
