...
public class Args extends CommonParameter {
  ...
  public static void setParam(final String[] args, final String confFileName) {
    ...
    PARAMETER.crossChainConnect = getCrossNodes(config);
  ...
  private static List<Node> getCrossNodes(final com.typesafe.config.Config config) {
    List<Node> ret = new ArrayList<>();
    PARAMETER.crossChainPort = config.hasPath(Constant.NODE_CROSS_CHAIN_PORT) ? config
    ...
    if (config.hasPath(Constant.NODE_CROSS_CHAIN_CONNECT)) {
      ...
      for (String configString : list) {
        ...
        if (!(PARAMETER.nodeDiscoveryBindIp.equals(n.getHost())
            ...
            || PARAMETER.nodeListenPort != n.getPort()
            || PARAMETER.backupPort != n.getPort()) {
          ret.add(n);
        }
      }
    }
  ...
  public static void logConfig() {
    ...
    logger.info("CrossChain node size: {}", parameter.getCrossChainConnect().size());
