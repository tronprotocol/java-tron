...
## Set the maximum memory usage of `FullNode.jar`
### Usage

Get the latest code from github

```
sh quickstart.sh -clone
```
...
```
sh quickstart.sh -download
```

### Download the latest version of the java-tron release
After successful execution, the following files will be automatically downloaded, `start.sh` can be used to start the `FullNode.jar` service
...
### Start the FullNode.jar via `start.sh`
...
Physical memory size in MB, here 12000 means 12000MB Start the service with `quickstart.sh`
