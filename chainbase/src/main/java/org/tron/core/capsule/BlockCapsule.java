...
public class BlockCapsule implements ProtoCapsule<Block> {
  ...
  public void setStateRoot(byte[] root) {
    this.block = this.block.toBuilder().setStateRoot(ByteString.copyFrom(root)).build();
  }
  ...
  public Sha256Hash getStateRoot() {
    if (this.block.getStateRoot() != null
        && !this.block.getStateRoot().isEmpty()) {
      return Sha256Hash.wrap(this.block.getStateRoot());
    }
    return Sha256Hash.ZERO_HASH;
  }
  ...
  public String toString() {
    ...
    toStringBuff.append("state root=").append(getStateRoot()).append("\n");
