...
public class BlockCapsule implements ProtoCapsule<Block> {
  ...
  public void setStateRoot(byte[] root) {
    BlockHeader blockHeader = this.block.getBlockHeader().toBuilder()
            .setStateRoot(ByteString.copyFrom(root)).build();
    this.block = this.block.toBuilder().setBlockHeader(blockHeader).build();
  }
  ...
  public Bytes32 getStateRoot() {
    ByteString root = this.block.getBlockHeader().getStateRoot();
    return root.isEmpty() ? MerklePatriciaTrie.EMPTY_TRIE_NODE_HASH :
