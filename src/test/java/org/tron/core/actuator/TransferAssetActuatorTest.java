...
public class TransferAssetActuatorTest {
  ...
  static {
    ...
    NOT_EXIT_ADDRESS_2 =
        Wallet.getAddressPreFixString() + "B56446E617E924805E4D6CA021D341FEF6E21234";
    ownerAsset_ADDRESS =
  ...
  private Any getContract(long sendCoin) {
    ...
    if (dbManager.getDynamicPropertiesStore().getAllowSameTokenName() == 1) {
      ...
      assetName = String.valueOf(tokenIdNum);
    }

    return Any.pack(Contract.TransferAssetContract.newBuilder()
        .setAssetName(ByteString.copyFrom(ByteArray.fromString(assetName)))
        .setOwnerAddress(ByteString.copyFrom(ByteArray.fromHexString(OWNER_ADDRESS)))
        .setToAddress(ByteString.copyFrom(ByteArray.fromHexString(TO_ADDRESS))).setAmount(sendCoin)
        .build());
  }
  ...
  private Any getContract(long sendCoin, byte[] toAddress) {
    ...
    return Any.pack(Contract.TransferAssetContract.newBuilder()
        .setAssetName(ByteString.copyFrom(ByteArray.fromString(assetName)))
        .setOwnerAddress(ByteString.copyFrom(ByteArray.fromHexString(OWNER_ADDRESS)))
        .setToAddress(ByteString.copyFrom(toAddress)).setAmount(sendCoin).build());
  }

  private Any getContract(long sendCoin, ByteString assetName) {
    return Any.pack(Contract.TransferAssetContract.newBuilder().setAssetName(assetName)
        .setOwnerAddress(ByteString.copyFrom(ByteArray.fromHexString(OWNER_ADDRESS)))
        .setToAddress(ByteString.copyFrom(ByteArray.fromHexString(TO_ADDRESS))).setAmount(sendCoin)
        .build());
  }
  ...
  private Any getContract(long sendCoin, String owner, String to) {
    ...
    return Any.pack(Contract.TransferAssetContract.newBuilder()
        ...
        .setToAddress(ByteString.copyFrom(ByteArray.fromHexString(to))).setAmount(sendCoin)
        .build());
  }

  private void createAssertBeforeSameTokenNameActive() {
  ...
  public void SameTokenNameCloseSuccessTransfer() {
    createAssertBeforeSameTokenNameActive();
  ...
  public void SameTokenNameCloseSuccessTransfer2() {
    ...
    try {
      ...
      Assert.assertEquals(toAccount.getInstance().getAssetMap().get(ASSET_NAME).longValue(),
          OWNER_ASSET_BALANCE);
      //check V2
      long tokenIdNum = dbManager.getDynamicPropertiesStore().getTokenIdNum();
      Assert.assertEquals(
  ...
  public void SameTokenNameCloseOwnerNoAssetTest() {
    ...
    try {
      ...
      Assert.assertEquals("Owner account has no asset!", e.getMessage());
      Assert.assertEquals(ret.getInstance().getRet(), code.SUCESS);
      AccountCapsule toAccount = dbManager.getAccountStore()
          .get(ByteArray.fromHexString(TO_ADDRESS));
  ...
  public void SameTokenNameCloseNoExitToAccount() {
    ...
    try {
      ...
      actuator.validate();
      actuator.execute(ret);
      noExitAccount = dbManager.getAccountStore().get(ByteArray.fromHexString(NOT_EXIT_ADDRESS));
  ...
  public void SameTokenNameOpenNoExitToAccount() {
    ...
    try {
      ...
      actuator.validate();
      actuator.execute(ret);
      noExitAccount = dbManager.getAccountStore().get(ByteArray.fromHexString(NOT_EXIT_ADDRESS_2));
      Assert.assertFalse(null == noExitAccount);    //Had created.
      Assert.assertEquals(noExitAccount.getBalance(), 0);
      actuator.execute(ret);
    } catch (ContractValidateException e) {
  ...
  public void SameTokenNameCloseOwnerNoThisAsset() {
    createAssertBeforeSameTokenNameActive();
    AccountCapsule ownerAssetCapsule = new AccountCapsule(
        ByteString.copyFrom(ByteArray.fromHexString(ownerAsset_ADDRESS)),
        ByteString.copyFromUtf8("ownerAsset"), AccountType.AssetIssue);
    ...
    try {
      ...
      Assert.assertEquals(owner.getAssetMap().get(ASSET_NAME).longValue(), OWNER_ASSET_BALANCE);
      Assert.assertTrue(isNullOrZero(toAccount.getAssetMap().get(ASSET_NAME)));
      AccountCapsule ownerAsset = dbManager.getAccountStore()
          .get(ByteArray.fromHexString(ownerAsset_ADDRESS));
  ...
  public void SameTokenNameOpenOwnerNoThisAsset() {
    ...
    AccountCapsule ownerAssetCapsule = new AccountCapsule(
        ...
        ByteString.copyFromUtf8("ownerAsset"), AccountType.AssetIssue);
    ownerAssetCapsule
        .addAssetV2(ByteArray.fromString(String.valueOf(tokenIdNum)), OWNER_ASSET_Test_BALANCE);

    AssetIssueContract assetIssueTestContract = AssetIssueContract.newBuilder()
        .setOwnerAddress(ByteString.copyFrom(ByteArray.fromHexString(ownerAsset_ADDRESS)))
        .setName(ByteString.copyFrom(ByteArray.fromString(ownerASSET_NAME)))
        .setTotalSupply(TOTAL_SUPPLY).setTrxNum(TRX_NUM).setId(String.valueOf(tokenIdNum))
        .setNum(NUM).setStartTime(START_TIME).setEndTime(END_TIME).setVoteScore(VOTE_SCORE)
        .setDescription(ByteString.copyFrom(ByteArray.fromString(DESCRIPTION)))
        .setUrl(ByteString.copyFrom(ByteArray.fromString(URL))).build();
    ...
    dbManager.getAssetIssueV2Store().put(assetIssueCapsule.createDbV2Key(), assetIssueCapsule);
    TransferAssetActuator actuator = new TransferAssetActuator(
        getContract(1, String.valueOf(tokenIdNum)), dbManager);
    TransactionResultCapsule ret = new TransactionResultCapsule();
    try {
      actuator.validate();
      actuator.execute(ret);
  ...
  public void transferToContractAddress()
    ...
    TransferAssetActuator actuator = new TransferAssetActuator(getContract(100L, contractAddress),
        dbManager);
    TransactionResultCapsule ret = new TransactionResultCapsule();
    try {
      actuator.validate();
