...
//      Assert.assertEquals("def", ByteArray.toStr(exchangeCapsule2.getSecondTokenId()));
  ...
  public void SameTokenNameOpenNoAccount() {
    ...
    try {
      actuator.validate();
      actuator.execute(ret);
      fail("account[+OWNER_ADDRESS_NOACCOUNT+] deos not exist");
    } catch (ContractValidateException e) {
      Assert.assertTrue(e instanceof ContractValidateException);
      Assert.assertEquals("account[" + OWNER_ADDRESS_NOACCOUNT + "] does not exist",
          e.getMessage());
  ...
  public void SameTokenNameCloseTokenIsNotInExchange() {
    ...
    try {
      ...
      Assert.assertTrue(e instanceof ContractValidateException);
      Assert.assertEquals("token is not in the exchange",
          e.getMessage());
    } catch (ContractExeException e) {
      Assert.assertFalse(e instanceof ContractExeException);
    } finally {
  ...
  public void SameTokenNameCloseTokenQuantLessThanZero() {
    ...
    try {
      ...
      Assert.assertTrue(e instanceof ContractValidateException);
      Assert.assertEquals("token quantity must be greater than 0",
          e.getMessage());
    } catch (ContractExeException e) {
      Assert.assertFalse(e instanceof ContractExeException);
    } finally {
  ...
  public void SameTokenNameOpenInvalidParam() {
    ...
    try {
      ...
      Assert.assertTrue(e instanceof ContractValidateException);
      Assert.assertEquals("token expected must be greater than 0",
          e.getMessage());
    } catch (ContractExeException e) {
      Assert.assertFalse(e instanceof ContractExeException);
    } finally {
