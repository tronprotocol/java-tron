...
public class UTXOSetTest {
  ...
  private Function<Wallet, TronTXOutput.TXOutput> addMockUTXO = (Wallet wallet) -> {
    TronTXOutput.TXOutput output = outputBuilder
        .setPubKeyHash(ByteString.copyFrom(wallet.getAddress())).build();
    byte[] key = generateKeyAndAddToMockKeySet.get();
    Mockito.when(mockTransactionDb.getData(key))
        .thenReturn(outputsBuilder.clearOutputs().addOutputs(output).build().toByteArray());
  ...
  public void testReturnAvailableSpendableOutputWhenInsufficientToCoverTheAmount() {
    ...
    outputBuilder.setValue(10);
    addMockUTXO.apply(testWallet);
    SpendableOutputs result = utxoSet
        .findSpendableOutputs(testWallet.getEcKey().getPubKey(), testAmount);
    assertEquals(outputBuilder.build().getValue(), result.getAmount());
    assertEquals(1, result.getUnspentOutputs().size());
  }
  ...
  public void testInvalidProtocolBufferExceptionInFindSpendableOutput() {
    ...
    assertTrue(
        outContent.toString().contains("com.google.protobuf.InvalidProtocolBufferException"));
  }
