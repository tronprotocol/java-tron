...
@Slf4j
public class codeSaftyUnsupport {
  ...
  private byte[] subcContractAddress = null;
  ...
  public void test01DeployTransferTokenContract() {
    ...
    String filePath = "./src/test/resources/soliditycode/codeSaftyUnsupport.sol";
    String contractName = "UseDot";
  ...
  public void test02DeployRevContract() {
    ...
    if (infoById.get().getResultValue() != 0) {
      Assert.fail(
          "deploy receive failed with message: " + infoById.get().getResMessage().toStringUtf8());
    }

    subcContractAddress = infoById.get().getContractAddress().toByteArray();

    SmartContract smartContract = PublicMethed
        .getContract(subcContractAddress, blockingStubFull);
    Assert.assertNotNull(smartContract.getAbi());

    Long contractAssetCount = PublicMethed.getAssetIssueValue(subcContractAddress,
        assetAccountId, blockingStubFull);
    logger.info("Contract has AssetId: " + assetAccountId.toStringUtf8() + ", Count: "
        + contractAssetCount);
  }
  ...
  public void test03Trigger5Contract() {
    ...
    Assert.assertTrue(PublicMethed.sendcoin(transferTokenContractAddress, 1000L,
    ...
    Long receiveAssetBefore = PublicMethed.getAssetIssueValue(subcContractAddress, assetAccountId,
        blockingStubFull);
    logger.info("before trigger, subcContractAddress has AssetId "
        + assetAccountId.toStringUtf8() + ", Count is " + receiveAssetBefore);
    long transferBalanceBefore = PublicMethed
        .queryAccount(transferTokenContractAddress, blockingStubFull)
        .getBalance();
    logger.info("before trigger, transferBalanceBefore: " + transferBalanceBefore);
    long resultBalanceBefore = PublicMethed.queryAccount(subcContractAddress, blockingStubFull)
        .getBalance();
    logger.info("before trigger, resultBalanceBefore: " + resultBalanceBefore);

    String tokenId = assetAccountId.toStringUtf8();
    Long tokenValue = Long.valueOf(2);
    Long callValue = Long.valueOf(0);

    String param = "\"" + Base58.encode58Check(subcContractAddress) + "\"";

    final String triggerTxid = PublicMethed.triggerContract(transferTokenContractAddress,
        "trigger5(address)", param, false, callValue,
    ...
    Long receiveAssetAfter = PublicMethed.getAssetIssueValue(subcContractAddress,
        assetAccountId, blockingStubFull);
    logger.info("after trigger, subcContractAddress has AssetId "
        + assetAccountId.toStringUtf8() + ", receiveAssetAfter is " + receiveAssetAfter);

    long transferBalanceAfter = PublicMethed
        .queryAccount(transferTokenContractAddress, blockingStubFull)
        .getBalance();
    logger.info("after trigger, transferBalanceAfter: " + transferBalanceAfter);

    long resultBalanceAfter = PublicMethed.queryAccount(subcContractAddress, blockingStubFull)
        .getBalance();
    logger.info("after trigger,  resultBalanceAfter: " + resultBalanceAfter);

    long consumeUserPercent = smartContract.getConsumeUserResourcePercent();
    logger.info("ConsumeURPercent: " + consumeUserPercent);

    Assert.assertEquals(Long.valueOf(transferAssetAfter - transferAssetBefore), tokenValue);
    Assert.assertEquals(resultBalanceAfter - resultBalanceBefore, 10);
  }

  @Test(enabled = true, description = "TransferToken with correct value, transfer to a contract")
  public void test04Trigger6Contract() {
    ...
    String param = "\"" + Base58.encode58Check(subcContractAddress) + "\"," + tokenId;

    final String triggerTxid = PublicMethed.triggerContract(transferTokenContractAddress,
        "trigger6(address,trcToken)", param, false, callValue,
        1000000000L, tokenId, tokenValue, user001Address, user001Key,
        blockingStubFull);
    PublicMethed.waitProduceNextBlock(blockingStubFull);
    ...
    Assert.assertEquals(Long.valueOf(transferAssetAfter - transferAssetBefore), tokenValue);
//    Assert.assertEquals(receiveAssetAfter - receiveAssetBefore, 10);
    Assert.assertEquals(resultBalanceAfter - resultBalanceBefore, 1000);
  }

  @Test(enabled = true, description = "TransferToken with correct value, get contract tokenBalance")
  public void test05TriggerGetBalanceContract() {
    ...
    Assert.assertTrue(transferBalanceAfter == ByteArray
