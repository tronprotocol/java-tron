...
public class codeSaftySupport {
  ...
  public void test01DeployTransferTokenContract() {
    ...
    String filePath = "./src/test/resources/soliditycode/codeSaftySupport.sol";
  ...
  public void test03TriggerContract() {
    ...
    long resultBalanceBefore = PublicMethed.queryAccount(resultContractAddress, blockingStubFull)
        .getBalance();
    logger.info("before trigger, resultBalanceBefore: " + resultBalanceBefore);

    String tokenId = assetAccountId.toStringUtf8();
    Long tokenValue = Long.valueOf(5);
    Long msgExpectTokenValue = Long.valueOf(2);
    Long callValue = Long.valueOf(10);

    String param = "\"" + Base58.encode58Check(resultContractAddress)
        + "\",\"" + tokenValue + "\"";

    final String triggerTxid = PublicMethed.triggerContract(transferTokenContractAddress,
        "transferToken(address,uint256)", param, false, callValue,
    ...
    Long msgCallValue = ByteArray.toLong(ByteArray.fromHexString(PublicMethed
        .getStrings(transactionInfo.getLogList().get(0).getData().toByteArray()).get(0)));
    Long msgTokenValue = ByteArray.toLong(ByteArray.fromHexString(PublicMethed
        .getStrings(transactionInfo.getLogList().get(1).getData().toByteArray()).get(0)));
    Long msgId = ByteArray.toLong(ByteArray.fromHexString(PublicMethed
        .getStrings(transactionInfo.getLogList().get(2).getData().toByteArray()).get(0)));
    ...
    long resultBalanceAfter = PublicMethed.queryAccount(resultContractAddress, blockingStubFull)
        .getBalance();
    ...
    Assert.assertEquals(msgExpectTokenValue, msgTokenValue);
    Assert.assertEquals(callValue, msgCallValue);

    Assert.assertEquals(Long.valueOf(receiveAssetAfter - receiveAssetBefore), msgExpectTokenValue);
    Assert.assertEquals(Long.valueOf(resultBalanceAfter - resultBalanceBefore), callValue);
  }
