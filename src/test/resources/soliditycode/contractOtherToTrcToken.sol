...
function stringToTrctoken(address toAddress, string memory tokenStr, uint256 tokenValue) public {
// trcToken t = trcToken(tokenStr); // ERROR
// toAddress.transferToken(tokenValue, tokenStr); // ERROR
}

function uint256ToTrctoken(address payable toAddress, uint256 tokenInt, uint256 tokenValue)  public {
trcToken t = trcToken(tokenInt); // OK
toAddress.transferToken(tokenValue, t); // OK
toAddress.transferToken(tokenValue, tokenInt); // OK
}

function addressToTrctoken(address payable toAddress, address adr, uint256 tokenValue) public {
...
function bytes32ToTrctoken(address payable toAddress, bytes32 b32, uint256 tokenValue) public {
trcToken t = trcToken(b32); // OK
toAddress.transferToken(tokenValue, t); // OK
// toAddress.transferToken(tokenValue, b32); // ERROR
}

function arrayToTrctoken(address payable toAddress, uint256 arr, uint256 tokenValue) public {
