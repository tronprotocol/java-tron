...
contract A{
    ...
    function testAssert(address bAddress,uint256 amount) public payable{
        bAddress.call.value(amount).gas(1000000)(abi.encode(bytes4(keccak256("callCGetZero(bool)")),false));//2.1
        bAddress.call.value(amount).gas(1000000)(abi.encode(bytes4(keccak256("callCGetZero(bool)")),true));
    }
    function testRequire(address cAddress,uint256 amount) public payable{
        cAddress.call.value(amount).gas(1000000)(abi.encode(bytes4(keccak256("newBAndTransfer(bool)")),false));//2.1
        cAddress.call.value(amount).gas(1000000)(abi.encode(bytes4(keccak256("newBAndTransfer(bool)")),true));
    }
