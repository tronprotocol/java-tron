...
public class GetTransactionByIdOnSolidityServlet
  ...
  protected void doGet(HttpServletRequest request, HttpServletResponse response) {
    try {
      ...
      Protocol.Transaction reply = walletOnSolidity.futureGet(() -> wallet
  ...
  protected void doPost(HttpServletRequest request, HttpServletResponse response) {
    try {
      ...
      Protocol.Transaction reply = walletOnSolidity.futureGet(
              () -> wallet.getTransactionById(build.getValue()));
      if (reply != null) {
        response.getWriter().println(Util.printTransaction(reply, visible));
      } else {
        response.getWriter().println("{}");
