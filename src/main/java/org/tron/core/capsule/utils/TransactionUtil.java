...
//    }
//
//    Set<Entry<String, long[]>> entrySet = spendableOutputs.getUnspentOutputs().entrySet();
//
...
//    if (spendableOutputs.getAmount() > amount) {
//      txOutputs.add(
//          TxOutputUtil.newTxOutput(spendableOutputs.getAmount() - amount,
...
//  }
  ...
  public static String toPrintString(Transaction transaction) {
    ...
    for (int i = 0, vinCount = transaction.getVinCount(); i < vinCount; i++) {
      ...
      sb.append("\t\t{\n" + "\t\t\ttxID=").append(ByteArray.toHexString(vin.getRawData().getTxID()
          .toByteArray())).append("\n").append("\t\t\tvout=").append(vin.getRawData().getVout())
          .append("\n")
          .append("\t\t\tsignature=").append(ByteArray.toHexString(vin
          .getSignature().toByteArray())).append("\n").append("\t\t\tpubKey=")
          .append(ByteArray.toHexString(vin.getRawData().getPubKey()
              .toByteArray())).append("\n").append("\t\t}");
...
//    if (TransactionUtil.isCoinbaseTransaction(signedTransaction)) {
//      return true;
//    }
//    //1. check hash
//    ByteString idByteS = signedTransaction.getId(); //hash
//    byte[] hash = TransactionUtil.getHash(signedTransaction);
...
//      TXInput vin = transactionBuilderBeforSign.getVin(i);
//      TXInput.Builder vinBuilder = vin.toBuilder();
//      ByteString pubKeyBs = signedTransaction.getVin(i).getRawData().getPubKey();
...
//        ByteString txId = vin.getRawData().getTxID();
//        int out = (int) (vin.getRawData().getVout());
...
//      byte[] hash = TransactionUtil.getHash(transactionBuilderForSign.build());
...
//    }
//    byte[] hash = TransactionUtil.getHash(transactionBuilderSigned.build());
