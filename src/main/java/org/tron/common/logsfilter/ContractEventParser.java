...
public class ContractEventParser {
  ...
  public static Map<String, Object> parseTopics(ContractEventTrigger trigger, ABI.Entry entry) {
    List<String> topicList = trigger.getTopicList();
    ...
    if (topicsMatched(trigger, entry)) {
      for (int i = 0; i < list.size(); ++i) {
        ...
        Object obj = parseTopic(ByteArray.fromHexString(topicList.get(index++)), param.getType());
        map.put(param.getName(), obj);
        map.put("" + i, obj);
      }
    } else {
      for (int i = 1; i < topicList.size(); ++i) {
        map.put("" + (i - 1), DataWord.shortHex(ByteArray.fromHexString(topicList.get(i))));
      }
    }
    return map;
  }
  ...
  public static Map<String, Object> parseEventData(ContractEventTrigger trigger, ABI.Entry entry) {
    ...
    // in case indexed topics doesn't match
    if (!topicsMatched(trigger, entry)) {
      map.put("" + (trigger.getTopicList().size() - 1), data);
      return map;
    }
    ...
    try {
      ...
      for (Integer i = 0; i < list.size(); ++i) {
        ...
        Object obj = parseDataBytes(ByteArray.fromHexString(data), param.getType(), index++);
        map.put(param.getName(), obj);
      ...
      map.clear();
      map.put(startIndex.toString(), data);
    }
    return map;
  }
